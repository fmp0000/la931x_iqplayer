...
l1_trace_host_code_t l1_trace_host_code[] = { { 0x100, "L1_TRACE_MSG_DMA_XFR " },
...
void print_host_trace(void) {
    ...
    for (i = 0, j = l1_trace_index; i < STATS_MAX; i++, j++) {
        ...
        k = 0;
        while (l1_trace_host_code[k].msg != 0xffff) {
            if (l1_trace_host_code[k].msg == entry->msg)
                break;
            k++;
        }
        printf("\n%4d, counter: %016ld (+%08ld ns), param: 0x%08x, (0x%03x)%s", i, entry->cnt,
               (entry->cnt - prev) * 1000000000 / rte_get_tsc_hz(), entry->param, entry->msg, l1_trace_host_code[k].text);
        prev = entry->cnt;
    }
    printf("\n");
